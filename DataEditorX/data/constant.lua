...
LOCATION_ONFIELD	=0x0c		--场上（LOCATION_MZONE+LOCATION_SZONE）（怪兽+魔陷）
LOCATION_PUBLIC		=0x3c		--（EDOPro）--LOCATION_ONFIELD+LOCATION_GRAVE+LOCATION_REMOVED
LOCATION_ALL		=0x3ff		--（EDOPro）
...
SEQ_DECKTOP			=0			--（EDOPro）
SEQ_DECKBOTTOM		=1			--（EDOPro）
SEQ_DECKSHUFFLE		=2			--（EDOPro）
--Positions 表示形式
...
TYPE_UNION			=0x400		--同盟
TYPE_DUAL			=0x800		--（YGOPro）二重
TYPE_GEMINI			=0x800		--（EDOPro）二重
...
TYPE_SKILL			=0x8000000	--（EDOPro）
TYPE_ACTION			=0x10000000	--（EDOPro）
TYPE_PLUS			=0x20000000	--（EDOPro）
TYPE_MINUS			=0x40000000	--（EDOPro）
TYPE_ARMOR			=0x80000000	--（EDOPro）
TYPES_TOKEN			=TYPE_MONSTER+TYPE_NORMAL+TYPE_TOKEN			--（EDOPro）
TYPE_EXTRA			=TYPE_FUSION+TYPE_SYNCHRO+TYPE_XYZ+TYPE_LINK	--（EDOPro）
--Attributes 属性
...
RACE_PYRO			=0x80		--炎
RACE_ROCK			=0x100		--岩石
RACE_WINDBEAST		=0x200		--（YGOPro）鸟兽
RACE_WINGEDBEAST	=0x200		--（EDOPro）鸟兽
RACE_PLANT			=0x400		--植物
...
RACE_PSYCHO			=0x100000	--（YGOPro）念动力
RACE_PSYCHIC		=0x100000	--（EDOPro）念动力
RACE_DIVINE			=0x200000	--幻神兽
RACE_CREATORGOD		=0x400000	--创造神
RACE_WYRM			=0x800000	--幻龙
RACE_CYBERSE		=0x1000000	--电子界
RACES_BEAST_BWARRIOR_WINGB	=RACE_BEAST+RACE_BEASTWARRIOR+RACE_WINGEDBEAST	--（EDOPro）
--Reason 卡片到当前位置的原因
...
LOCATION_REASON_COUNT		=0x4	--（EDOPro）
--Summon Type --召唤类型
SUMMON_TYPE_NORMAL		=0x10000000 --通常召唤(EFFECT_SUMMON_PROC,EFFECT_SET_PROC 可用Value修改數值)
SUMMON_TYPE_ADVANCE		=0x11000000 --（YGOPro）--上级召唤
SUMMON_TYPE_TRIBUTE		=0x11000000	--（EDOPro）--上级召唤
SUMMON_TYPE_DUAL		=0x12000000	--（YGOPro）--再度召唤（二重）
SUMMON_TYPE_GEMINI		=0x12000000	--（EDOPro）--再度召唤（二重）
...
ASSUME_ATTACK		=7
ASSUME_DEFENSE		=8
ASSUME_LINK         =9  --（EDOPro）
ASSUME_LINKMARKER   =10 --（EDOPro）
--Link Marker
...
RESETS_STANDARD_DISABLE		=RESETS_STANDARD|RESET_DISABLE	--（EDOPro）
RESETS_REDIRECT				=(RESETS_STANDARD|RESET_OVERLAY)&~(RESET_TOFIELD|RESET_LEAVE) --(EFFECT_LEAVE_FIELD_REDIRECT)
RESETS_CANNOT_ACT			=RESETS_STANDARD&~RESET_LEAVE	--（EDOPro）
...
EFFECT_FLAG_COF				=0x20000000 --（YGOPro）N/A
EFFECT_FLAG_CVAL_CHECK		=0x40000000	--（YGOPro）以卡为COST的诱发效果需要使用
...
EFFECT_FLAG2_MAJESTIC_MUST_COPY	=0x80000000	--（EDOPro）
...
EFFECT_QP_ACT_IN_SET_TURN		=19		--（EDOPro）
...
EFFECT_DEVINE_LIGHT				=38		--（YGOPro）--神圣光辉（魔术礼帽）
EFFECT_LIGHT_OF_INTERVENTION	=38		--（EDOPro）--神圣光辉（魔术礼帽）
...
EFFECT_DUAL_STATUS				=75		--（YGOPro）--二重状态
EFFECT_GEMINI_STATUS			=75		--（EDOPro）--二重状态
EFFECT_EQUIP_LIMIT				=76		--装备对象限制
EFFECT_DUAL_SUMMONABLE			=77		--（YGOPro）--可以再度召唤
EFFECT_GEMINI_SUMMONABLE		=77		--（EDOPro）--可以再度召唤
...
EFFECT_CHANGE_BATTLE_STAT		=198	--（EDOPro）
...
EFFECT_TOSS_COIN_CHOOSE			=222	--（EDOPro）
EFFECT_TOSS_DICE_CHOOSE			=223	--（EDOPro）
...
EFFECT_SCRAP_CHIMERA				=246--（YGOPro）--废铁奇美拉
EFFECT_TUNE_MAGICIAN_X				=247--（YGOPro）--调弦之魔术师超量素材限制
EFFECT_TUNE_MAGICIAN_F				=248--（YGOPro）--调弦之魔术师融合素材限制
EFFECT_CANNOT_BE_MATERIAL			=248--（EDOPro）
...
EFFECT_MUST_USE_MZONE			=265	--（YGOPro）--必须使用怪兽区的格子
EFFECT_FORCE_MZONE				=265	--（EDOPro）
EFFECT_BECOME_LINKED_ZONE		=266	--（EDOPro）
...
EFFECT_MUST_BE_SMATERIAL		=312	--（YGOPro）--必须作为同调素材（波动龙 声子龙）
EFFECT_MUST_BE_MATERIAL			=312	--（EDOPro）--必须作为同调素材（波动龙 声子龙）
...
EFFECT_DISABLE_CHAIN_FIELD		=337	--（YGOPro）--N/A
EFFECT_DISCARD_COST_CHANGE		=338	--反制陷阱捨棄手牌的代價改變(解放之阿里阿德涅)
EFFECT_HAND_SYNCHRO				=339	--用手牌的怪獸當作同步素材
EFFECT_ADD_FUSION_CODE			=340	--（YGOPro）--作为融合素材时可以当作某一卡名(融合识别)
EFFECT_ADD_FUSION_SETCODE		=341	--（YGOPro）--作为融合素材时可以当作某一字段(魔玩具改造)
EFFECT_RISE_TO_FULL_HEIGHT		=342	--（YGOPro）--仁王立
...
EFFECT_ADD_FUSION_ATTRIBUTE		=349	--（YGOPro）--reserve
EFFECT_REMOVE_SETCODE			=349	--（EDOPro）
EFFECT_REMOVE_FUSION_ATTRIBUTE	=350	--（YGOPro）--reserve
EFFECT_CHANGE_SETCODE			=350	--（EDOPro）
EFFECT_CHANGE_FUSION_ATTRIBUTE	=351	--（YGOPro）--用作融合素材时的属性
EFFECT_EXTRA_FUSION_MATERIAL	=352	--增加融合素材(万溶炼金师)
EFFECT_TUNER_MATERIAL_LIMIT		=353	--同调素材限制
EFFECT_ADD_LINK_CODE			=354	--（YGOPro）--用作连接素材时的卡名
EFFECT_ADD_LINK_SETCODE			=355	--（YGOPro）--reserve
EFFECT_ADD_LINK_ATTRIBUTE		=356	--（YGOPro）--用作连接素材时的属性
EFFECT_ADD_LINK_RACE			=357	--（YGOPro）--用作连接素材时的种族
EFFECT_EXTRA_MATERIAL			=358	--（EDOPro）
EFFECT_EXTRA_LINK_MATERIAL		=358	--（YGOPro）--手卡的连接素材
EFFECT_QP_ACT_IN_SET_TURN		=359	--（YGOPro）--速攻魔法可以在盖放的回合发动
EFFECT_EXTRA_PENDULUM_SUMMON	=360	--extra pendulum summon
EFFECT_MATERIAL_LIMIT			=361	--（YGOPro）
EFFECT_IRON_WALL				=361	--（EDOPro）
EFFECT_SET_BATTLE_ATTACK		=362    --（YGOPro）--战斗的伤害计算用设置的攻击力进行
EFFECT_SET_BATTLE_DEFENSE		=363    --（YGOPro）--战斗的伤害计算用设置的守备力进行
--EDOPro effects
EFFECT_CANNOT_LOSE_DECK			=400	--（EDOPro）
EFFECT_CANNOT_LOSE_LP			=401	--（EDOPro）
EFFECT_CANNOT_LOSE_EFFECT		=402	--（EDOPro）
EFFECT_BP_FIRST_TURN			=403	--（EDOPro）
EFFECT_UNSTOPPABLE_ATTACK		=404	--（EDOPro）
EFFECT_ALLOW_NEGATIVE			=405	--（EDOPro）
EFFECT_SELF_ATTACK				=406	--（EDOPro）
EFFECT_BECOME_QUICK				=407	--（EDOPro）
EFFECT_LEVEL_RANK				=408	--（EDOPro）
EFFECT_RANK_LEVEL				=409	--（EDOPro）
EFFECT_LEVEL_RANK_S				=410	--（EDOPro）
EFFECT_RANK_LEVEL_S				=411	--（EDOPro）
EFFECT_UPDATE_LINK              =420	--（EDOPro）
EFFECT_CHANGE_LINK              =421	--（EDOPro）
EFFECT_CHANGE_LINK_FINAL        =422	--（EDOPro）
EFFECT_ADD_LINKMARKER           =423	--（EDOPro）
EFFECT_REMOVE_LINKMARKER        =424	--（EDOPro）
EFFECT_CHANGE_LINKMARKER        =425	--（EDOPro）
EFFECT_FORCE_NORMAL_SUMMON_POSITION	=426	--（EDOPro）
EFFECT_FORCE_SPSUMMON_POSITION		=427	--（EDOPro）
EFFECT_DARKNESS_HIDE				=428	--（EDOPro）
EFFECT_FUSION_MAT_RESTRICTION		=73941492+TYPE_FUSION	--（EDOPro）
EFFECT_SYNCHRO_MAT_RESTRICTION		=73941492+TYPE_SYNCHRO	--（EDOPro）
EFFECT_XYZ_MAT_RESTRICTION			=73941492+TYPE_XYZ		--（EDOPro）

--KoishiPro effects
EFFECT_CHANGE_LINK_MARKER_KOISHI			=710253		--（KoishiPro）
EFFECT_ADD_LINK_MARKER_KOISHI				=37564151	--（KoishiPro）
EFFECT_REMOVE_LINK_MARKER_KOISHI			=37564152	--（KoishiPro）
EFFECT_CANNOT_LOSE_KOISHI					=37564153	--（KoishiPro）
EFFECT_EXTRA_TOMAIN_KOISHI					=37564154	--（KoishiPro）
EFFECT_OVERLAY_REMOVE_COST_CHANGE_KOISHI	=37564155	--（KoishiPro）
EFFECT_ALLOW_SYNCHRO_KOISHI					=37564156	--（KoishiPro）
EFFECT_MINIATURE_GARDEN_GIRL				=37564157	--（KoishiPro）
EFFECT_ADD_SUMMON_TYPE_KOISHI				=37564158	--（KoishiPro）
EFFECT_REMOVE_SUMMON_TYPE_KOISHI			=37564159	--（KoishiPro）
EFFECT_CHANGE_SUMMON_TYPE_KOISHI			=37564160	--（KoishiPro）
EFFECT_CHANGE_SUMMON_LOCATION_KOISHI		=37564161	--（KoishiPro）
EFFECT_LINK_SPELL_KOISHI					=37564162	--（KoishiPro）
EFFECT_SEA_PULSE							=37564163	--（KoishiPro）
EFFECT_MAP_OF_HEAVEN						=77702007	--（KoishiPro）
...
EVENT_CHAIN_ACTIVATED			=1023	--（YGOPro）--N/A
...
EVENT_RETURN_TO_GRAVE			=1203	--（YGOPro）--回到墓地时
...
CATEGORY_GRAVE_ACTION		=0x800  	--（YGOPro）--包含涉及墓地的部分的效果（屋敷わらし）
CATEGORY_FLIP				=0x800  	--（EDOPro）--含翻转效果
...
HINT_CARD				=10
HINT_ZONE				=11
HINT_SKILL				=200	--（EDOPro）
HINT_SKILL_COVER		=201	--（EDOPro）
HINT_SKILL_FLIP			=202	--（EDOPro）
HINT_SKILL_REMOVE		=203	--（EDOPro）
--custom hints in KoishiPro for custom sound
HINT_MUSIC				=21		--（KoishiPro）
HINT_SOUND				=22		--（KoishiPro）
HINT_MUSIC_OGG			=23		--（KoishiPro）
--Card Hint
CHINT_TURN				=1
CHINT_CARD				=2
CHINT_RACE				=3
...
OPCODE_BAND				=0x40000008	--（EDOPro）
OPCODE_BOR				=0x40000009	--（EDOPro）
OPCODE_BNOT				=0x40000010	--（EDOPro）
OPCODE_BXOR				=0x40000011	--（EDOPro）
OPCODE_LSHIFT			=0x40000012	--（EDOPro）
OPCODE_RSHIFT			=0x40000013	--（EDOPro）
...
OPCODE_GETCODE			=0x40000105	--（EDOPro）
OPCODE_GETSETCARD		=0x40000106	--（EDOPro）
OPCODE_GETTYPE			=0x40000107	--（EDOPro）
OPCODE_GETRACE			=0x40000108	--（EDOPro）
OPCODE_GETATTRIBUTE		=0x40000109	--（EDOPro）
...
HINGMSG_NUMBER			=565	--（YGOPro）--请选择一个数字
HINTMSG_NUMBER			=565	--（EDOPro）--请选择一个数字
HINTMSG_EFFACTIVATE		=566	--（EDOPro）
HINGMSG_LVRANK			=567	--（YGOPro）--请宣言一个等级
HINTMSG_LVRANK			=567	--（EDOPro）--请宣言一个等级
...
HINTMSG_DISABLE			=573	--（YGOPro）--请选择要无效的卡
HINTMSG_OPERATECARD		=574	--（YGOPro）--请选择要操作的卡
HINTMSG_NEGATE			=575	--（EDOPro）
--Select	--请选择
SELECT_HEADS				=60	--正面
SELECT_TAILS				=61	--反面
...
GLOBALFLAG_TUNE_MAGICIAN		=0x400		--（YGOPro）--超量素材检查标记（调弦之魔术师）
--count_code
EFFECT_COUNT_CODE_OATH			=0x10000000	--发动次数限制(誓约次数, 发动被无效不计数)
EFFECT_COUNT_CODE_DUEL			=0x20000000	--决斗中使用次数
EFFECT_COUNT_CODE_SINGLE		=0x1		--（YGOPro）--同一张卡的多个效果公共使用次数
EFFECT_COUNT_CODE_SINGLE		=0x40000000	--（EDOPro）
--特殊选项
DUEL_TEST_MODE			=0x01		--测试模式(目前暫無)
DUEL_ATTACK_FIRST_TURN	=0x02		--第一回合可以攻击(用于残局)
DUEL_NO_CHAIN_HINT		=0x04		--（YGOPro）--N/A
DUEL_OBSOLETE_RULING	=0x08		--使用舊規則
DUEL_PSEUDO_SHUFFLE		=0x10		--不洗牌
DUEL_TAG_MODE			=0x20		--（YGOPro）--双打PP
DUEL_SIMPLE_AI			=0x40		--AI(用于残局)
DUEL_RELAY							=0x80		--（EDOPro）
DUEL_OBSOLETE_IGNITION				=0x100		--（EDOPro）
DUEL_1ST_TURN_DRAW					=0x200		--（EDOPro）
DUEL_1_FIELD						=0x400		--（EDOPro）
--internal: DUEL_1_FACEUP_FIELD		=0x400		--（EDOPro）
DUEL_PZONE							=0x800		--（EDOPro）
DUEL_SEPARATE_PZONE					=0x1000		--（EDOPro）
DUEL_EMZONE							=0x2000		--（EDOPro）
DUEL_FSX_MMZONE						=0x4000		--（EDOPro）
DUEL_TRAP_MONSTERS_NOT_USE_ZONE		=0x8000		--（EDOPro）
DUEL_RETURN_TO_EXTRA_DECK_TRIGGERS	=0x10000	--（EDOPro）
DUEL_TRIGGER_ONLY_IN_LOCATION		=0x20000	--（EDOPro）
DUEL_SPSUMMON_ONCE_OLD_NEGATE		=0x40000	--（EDOPro）
DUEL_CANNOT_SUMMON_OATH_OLD			=0x80000	--（EDOPro）
DUEL_NO_STANDBY_PHASE				=0x100000	--（EDOPro）
DUEL_NO_MAIN_PHASE_2				=0x200000	--（EDOPro）
DUEL_3_COLUMNS_FIELD				=0x400000	--（EDOPro）
DUEL_DRAW_UNTIL_5					=0x800000	--（EDOPro）
DUEL_NO_HAND_LIMIT					=0x1000000	--（EDOPro）
DUEL_UNLIMITED_SUMMONS				=0x2000000	--（EDOPro）
DUEL_INVERTED_QUICK_PRIORITY		=0x4000000	--（EDOPro）
--Compost flags for duel modes（EDOPro）
